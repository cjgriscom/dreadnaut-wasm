...
#define SIGALTSTACK_DEC 1
                /* sigaltstack() declared with _POSIX_C_SOURCE */
...
#if HAVE_FSEEKO && !FSEEKO_DEC
int fseeko(FILE *stream, off_t offset, int whence);
off_t ftello(FILE *stream);
#endif

#if HAVE_POPEN && !POPEN_DEC
...
char *gtools_getline(FILE*);     /* formerly getline() */
int graphsize(char*);
...
NORET_ATTR void gt_abort(const char*);
char *stringcopy(char*);
boolean strhaschar(char*,int);

void fcanonise(graph*,int,int,graph*,char*,boolean);
void fcanonise_inv(graph*,int,int,graph*,char*,void(*)(graph*,int*,int*,int,
